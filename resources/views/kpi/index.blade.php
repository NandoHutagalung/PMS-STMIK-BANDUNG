<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-slate-800">Data KPI</h2>
                <p class="text-sm text-slate-500 mt-1">Kelola indikator kinerja yang digunakan dalam penilaian.</p>
            </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6" x-data="{ q: '' }">
        ...
        <div class="relative mb-5 max-w-sm">
            ...
            <input type="text" x-model="q" placeholder="Cari kode atau nama KPI..."
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-blue-50 text-blue-900 text-xs uppercase tracking-wide">
                        ...
                        <th class="px-4 py-3 text-left">Kode KPI</th>
                        <th class="px-4 py-3 text-left">Nama KPI</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr x-show="!q || $el.innerText.toLowerCase().includes(q.toLowerCase())" class="hover:bg-blue-50/40">
                        ...
                        <td class="px-4 py-3 font-medium text-slate-700">{{ $kpi->kode_kpi }}</td>
                        <td class="px-4 py-3 text-slate-700">{{ $kpi->nama_kpi }}</td>
                        <td class="px-4 py-3 text-slate-700">{{ $kpi->bobot }}%</td>
                        <td class="px-4 py-3 text-slate-500 max-w-xs truncate">{{ $kpi->deskripsi }}</td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('kpi.destroy', $kpi->id) }}" method="POST"
                                      onsubmit="return confirm('Hapus KPI ini?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-slate-400">Belum ada data KPI.</td>
                    </tr>
                    @endforelse
