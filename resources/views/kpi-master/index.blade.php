    ...
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">Kelola daftar {{ strtolower($judul) }} yang bisa dipilih saat membuat KPI.</p>
            </div>
            <a href="{{ route('kpi-master.create', $slug) }}"
               class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white text-sm font-semibold rounded-lg shadow-sm hover:bg-blue-700 transition">
                <x-icon name="plus" class="w-4 h-4" /> Tambah {{ $judul }}
            </a>
        </div>
    </x-slot>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-blue-50 text-blue-900 text-xs uppercase tracking-wide">
                        ...
                        <th class="px-4 py-3 text-left">Satuan Default</th>
                        @endif
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr x-show="!q || $el.innerText.toLowerCase().includes(q.toLowerCase())" class="hover:bg-blue-50/40">
                        ...
                        @if($slug == 'indikator')
                        <td class="px-4 py-3 text-slate-600">{{ $item->parent->nama ?? '-' }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $item->satuan_default ?? '-' }}</td>
                        @endif
                        <td class="px-4 py-3 text-slate-500 max-w-xs truncate">{{ $item->deskripsi ?? '-' }}</td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('kpi-master.edit', [$slug, $item->id]) }}"
                                ...
                                <form action="{{ route('kpi-master.destroy', [$slug, $item->id]) }}" method="POST"
                                      onsubmit="return confirm('Hapus {{ strtolower($judul) }} ini?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                    ...
                    @empty
                    <tr>
                        <td colspan="{{ $slug == 'indikator' ? 5 : 4 }}" class="px-4 py-8 text-center text-slate-400">
