<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-slate-800">Data Periode Penilaian</h2>
                <p class="text-sm text-slate-500 mt-1">Kelola periode penilaian kinerja.</p>
            </div>
            <a href="{{ route('periode.create') }}"
               class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white text-sm font-semibold rounded-lg shadow-sm hover:bg-blue-700 transition">
                <x-icon name="plus" class="w-4 h-4" /> Tambah Periode
            </a>
        </div>
    </x-slot>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr x-show="!q || $el.innerText.toLowerCase().includes(q.toLowerCase())" class="hover:bg-blue-50/40">
                        ...
                        <td class="px-4 py-3 font-medium text-slate-700">{{ $periode->nama_periode }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $periode->tahun }}</td>
                        <td class="px-4 py-3">
                            <x-badge :color="$periode->status == 'Aktif' ? 'green' : 'gray'">
                                {{ $periode->status }}
                            </x-badge>
                        </td>
                        <td class="px-4 py-3">
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-slate-400">Belum ada data periode.</td>
                    </tr>
                    @endforelse
