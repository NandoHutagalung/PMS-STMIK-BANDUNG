<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-slate-800">Tambah Capaian Kinerja</h2>
        <p class="text-sm text-slate-500 mt-1">Catat realisasi pekerjaan berdasarkan KPI yang dimiliki.</p>
    </x-slot>

    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 max-w-3xl">
        <form action="{{ route('capaian.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <x-text-input id="pegawai" name="pegawai" type="text" class="w-full" value="{{ old('pegawai') }}" />
                ...
                <div>
                    ...
                    <x-text-input id="target" name="target" type="number" class="w-full" value="{{ old('target') }}" />
                    <x-input-error :messages="$errors->get('target')" class="mt-1.5" />
                </div>

                <div>
                    <x-input-label for="realisasi" value="Realisasi" />
                    <x-text-input id="realisasi" name="realisasi" type="number" class="w-full" value="{{ old('realisasi') }}" />
