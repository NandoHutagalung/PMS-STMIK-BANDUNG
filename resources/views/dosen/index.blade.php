<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">Kelola data dosen STMIK Bandung.</p>
            </div>
            <a href="{{ route('dosen.create') }}"
               class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white text-sm font-semibold rounded-lg shadow-sm hover:bg-blue-700 transition">
                <x-icon name="plus" class="w-4 h-4" /> Tambah Dosen
            </a>
        </div>
    </x-slot>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6" x-data="{ q: '' }">
        ...
        <div class="relative mb-5 max-w-sm">
            ...
            <input type="text" x-model="q" placeholder="Cari nama atau NIDN..."
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr x-show="!q || $el.innerText.toLowerCase().includes(q.toLowerCase())" class="hover:bg-blue-50/40">
                        <td class="px-4 py-3 font-medium text-slate-700">{{ $dosen->nama }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $dosen->nidn }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $dosen->jabatan }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $dosen->program_studi }}</td>
                        <td class="px-4 py-3">
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-slate-400">Belum ada data dosen.</td>
                    </tr>
                    @endforelse
