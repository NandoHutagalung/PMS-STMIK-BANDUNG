<x-app-layout>
    ...
    <div x-data="kpiTemplateForm()" class="space-y-6">
        ...
        <form action="{{ route('kpi-template.store') }}" method="POST" @submit="beforeSubmit">
            @csrf
            ...
            <x-card title="Form Input KPI" subtitle="Tentukan kategori, unit kerja, jabatan, dan periode." icon="presentation">
                ...
                <div>
                    ...
                    <div>
                        ...
                        <x-text-input id="unit_kerja" name="unit_kerja" type="text" class="w-full" value="{{ old('unit_kerja') }}" placeholder="Contoh: Pusat Layanan Teknologi Informasi" />
                        <x-input-error :messages="$errors->get('unit_kerja')" class="mt-1.5" />
                    </div>

                    <div>
                        <x-input-label for="jabatan" value="Jabatan" />
                        <x-text-input id="jabatan" name="jabatan" type="text" class="w-full" value="{{ old('jabatan') }}" placeholder="Contoh: Staf Sistem Informasi" />
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button type="submit" name="status" value="draft"
                        ...
                        <x-icon name="check-circle" class="w-4 h-4" /> Simpan Draft
                    </button>
                    <button type="submit" name="status" value="diajukan"
                        ...
                        <x-icon name="check-circle" class="w-4 h-4" /> Simpan & Ajukan
                    </button>
                </div>
            </div>
    ...
    <script>
        function kpiTemplateForm() {
            return {
                rows: [
                    { key: Date.now(), aspek: '', indikator: '', deskripsi: '', target: 0, satuan: '', bobot: 0 },
                ],
                addRow() {
